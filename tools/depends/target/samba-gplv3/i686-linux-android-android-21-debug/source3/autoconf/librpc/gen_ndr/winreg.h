...
#else
 { __do_not_use_enum_winreg_CreateAction=0x7FFFFFFF}
...
struct winreg_StringBuf {
	...
	uint16_t size;
	const char *name;/* [size_is(size/2),length_is(length/2),unique,charset(UTF16)] */
};
...
struct winreg_EnumValue {
	struct {
		...
		uint8_t *value;/* [unique,length_is(length?*length:0),range(0,0x4000000),size_is(size?*size:0)] */
...
struct winreg_QueryValue {
	struct {
		...
		enum winreg_Type *type;/* [unique] */
		uint8_t *data;/* [range(0,0x4000000),length_is(data_length?*data_length:0),unique,size_is(data_size?*data_size:0)] */
...
struct winreg_QueryMultipleValues2 {
	...
	struct {
		struct QueryMultipleValue *values_out;/* [length_is(num_values),ref,size_is(num_values)] */
		uint32_t *needed;/* [ref] */
		uint8_t *buffer;/* [unique,length_is(*offered),size_is(*offered)] */
		WERROR result;
	} out;

...
struct netr_USER_PRIVATE_INFO {
	...
	uint32_t DataLength;
	uint8_t *SensitiveData;/* [size_is(DataLength),flag(LIBNDR_PRINT_ARRAY_HEX),unique] */
};
...
struct netr_CIPHER_VALUE {
	uint32_t len;
	uint32_t maxlen;
	uint8_t *cipher_data;/* [size_is(maxlen),length_is(len),unique] */
};
...
union netr_DELTA_ID_UNION {
	...
	const char *name;/* [case(NETR_DELTA_SECRET),unique,charset(UTF16)] */
}/* [switch_type(netr_DeltaEnum)] */;
...
#else
 { __do_not_use_enum_netr_TrustType=0x7FFFFFFF}
...
struct netr_LogonGetDomainInfo {
	...
	struct {
		union netr_DomainInfo *info;/* [switch_is(level),ref] */
		struct netr_Authenticator *return_authenticator;/* [ref] */
		NTSTATUS result;
	} out;
